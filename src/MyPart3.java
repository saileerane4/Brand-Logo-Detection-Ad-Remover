...
public class MyPart3 {
	...
	public static void analyzeVideo(String videopath, int[][][] logos, MyPart2.Shot[] shots) throws IOException {
		//Turn file into path
		...
		for (curFrame = 1; numRead != -1; ++curFrame) {
			...
			for (int i = 0; i < countStarG.length; ++i) {
				if (countStarG[i] > 2000 && countStarW[i] > 800 && countStarW[i] < 4000) {
					++shots[curShot].logos[MyPart2.Logo.STARBUCKS.key];
					//Simple break;
					i = countStarG.length;
			...
			for (int i = 0; i < countSubY.length; ++i) {
				if (countSubY[i] >= 2200 && countSubW[i] >= 2500 && countSubW[i] < 4500) {
					...
					if (mse < 20000) {
						...
						++shots[curShot].logos[MyPart2.Logo.SUBWAY.key];
						//Simple break;
						i = countSubY.length;
					}
				}
			...
			for (int i = 0; i < countNFLB.length; ++i) {
				if (countNFLB[i] > 3000 && countNFLR[i] > 1000 && countNFLW[i] > 1000) {
					...
					++shots[curShot].logos[MyPart2.Logo.NFL.key];
					//Simple break;
					i = countNFLB.length;
				}
			}
			...
			for (int i = 0; i < countMcdY.length; ++i) {
				if (countMcdY[i] > 1500 && countMcdR[i] > 1000) {
					...
					++shots[curShot].logos[MyPart2.Logo.MCDONALDS.key];
					//Simple break;
					i = countMcdY.length;
				}
			}
	...
	public static void main(String[] args) throws IOException {
		//int[][][] logos = readLogos();
		//testImage(logos[0]);
		//analyzeVideo("dataset2/Videos/data_test2.rgb", logos, null);
		//testImage(logos[0]);
	}
